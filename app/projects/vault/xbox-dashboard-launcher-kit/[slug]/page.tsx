import { notFound } from "next/navigation";
import { dashboardLauncherKitDocuments } from "@/content/projects/vault/xbox-dashboard-launcher-kit/meta";

type PageProps = {
  params: Promise<{
    slug: string;
  }>;
};

export function generateStaticParams() {
  return dashboardLauncherKitDocuments.map((doc) => ({
    slug: doc.slug,
  }));
}

export default async function Page({ params }: PageProps) {
  const { slug } = await params;

  const doc = dashboardLauncherKitDocuments.find((d) => d.slug === slug);
  if (!doc) return notFound();

  const filename = doc.mdx.split("/").pop();
  if (!filename) return notFound();

  const mod = await import(
    `@/content/projects/vault/xbox-dashboard-launcher-kit/documents/${filename}`
  );
  const Content = mod.default;

  return (
    <main className="mx-auto max-w-3xl px-6 py-12 md:py-20 space-y-8">
      <article className="prose prose-vault max-w-none">
        <Content />
      </article>

      {doc.pdf && (
        <a
          href={doc.pdf}
          download
          className="text-green-400 hover:text-green-300 underline block mt-6 md:mt-8"
        >
          Download PDF
        </a>
      )}
    </main>
  );
}